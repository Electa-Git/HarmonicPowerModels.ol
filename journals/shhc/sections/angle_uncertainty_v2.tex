...
\subsection{Dependency Analysis and Probability Density Function Fitting}
...
An important feature of the harmonics generated by LCC is the inter-harmonic dependency due to frequency coupling as indicated by the HHS model in Equations~\eqref{eq:HSS}. To assess this dependency from the probabilistic distributions obtained via MCS, the correlation structure between different harmonic components is investigated. Correlation matrices, with Kendall's Tau correlation coefficients, are used to characterize the dependencies.

As shown in Fig.~\ref{fig:Correlation_Matrix_Copula}, the phase angle correlation matrix $\mathbf{R}_{\text{phase}}$ exhibits very strong correlations with the Gaussian copula parameters $\rho_{ij}^{Gauss} \geq 0.97$, indicating distinct characteristics in the inter-harmonic dependency structure for phase angles.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\columnwidth]{figures/Correlation_Matrix_Copula.png}
    \caption{Gaussian copula correlation matrices for harmonic magnitudes and harmonic phase angles.}
    \label{fig:Correlation_Matrix_Copula}
\end{figure}

\subsubsection{Marginal Distribution Fitting}
The marginal distributions of each harmonic component are modeled independently. The best-fit PDF for each harmonic magnitude and phase angle is fitted to the data obtained from MCS. The candidate distributions considered include Normal, Lognormal, Beta, Logistic and Exponential distributions.

For each harmonic $h$, the marginal distribution $F_h(x_h)$ is estimated by fitting multiple parametric distributions and selecting the best-fit  distribution based on minimizing the Root Mean Squared Error (RMSE) between the cumulative distribution functions simulated and fitted. The best-fit results are illustrated in Fig.\ref{fig:Bestfit_PDF}, showing the histograms and the fitted PDFs of harmonic magnitudes and phase angles.
...
Since strong inter-harmonic dependencies are proved for phase angles, a shared Beta distribution is used to model the phase angle distributions of all harmonic jointly. In this way, only two shape parameters need to be estimated for all harmonics, while each harmonic retains its own location and scale parameters to capture the specific range of phase angles.

Firstly, a normalization procedure is applied to each harmonic's unwrepped phase angle data to map them onto the unit interval $[0,1]$. Let $\phi_h = \{\phi_{h,i}\}_{i=1}^{N}$ denote the unwrapped phase samples in degrees for harmonic $h \in \{1, 5, 7, 11, 13\}$, where unwrapping is performed to avoid discontinuities at $\pm 180^\circ$. For each harmonic, the observed min–max values are represented as:
\begin{equation}
v_{\min,h} = \min(\phi_h), \qquad v_{\max,h} = \max(\phi_h).
\end{equation}
The sampled data of each harmonic phase angle is then normalized to the unit interval via the min-max transformation:
\begin{equation}
u_{h,i} = \frac{\phi_{h,i} - v_{\min,h}}{v_{\max,h} - v_{\min,h}} \in (0,1).
\end{equation}
% with optional clipping by a small tolerance $\varepsilon = 10^{-6}$ to avoid numerical issues at the boundaries $\{0, 1\}$.
...
The result of the fitted parameters are $\hat{\alpha} = 4.0414$, $\hat{\beta} = 4.7762$.
...
Thirdly, the fitted shared Beta distribution is transformed back to each harmonic's sampled phase angle scale to assess the goodness-of-fit. For each harmonic $h$, the denormalization is performed as:
\begin{equation}
...
Table~\ref{tab:beta_phase_params} lists the denormalized parameters (location $v_{\min,h}$ and scale $v_{\max,h} - v_{\min,h}$) for each harmonic. These parameters define the linear mapping from the standard $[0,1]$ interval back to the sampled phase ranges. The shared shape parameters $\hat{\alpha}$ and $\hat{\beta}$ remain constant across all harmonics. 

The goodness-of-fit of the shared Beta distribution model is evaluated for each harmonic phase angle by comparing the denormalized Beta PDF against the empirical histograms from MCS. Root Mean Squared Error (RMSE) is computed between the empirical histogram and the denormalized Beta PDF, with the results summarized in last column of Table~\ref{tab:beta_phase_params}.
...
\hline
Harmonic & $\hat{\alpha}$ & $\hat{\beta}$ & $v_{\min}$ [deg] & $v_{\max}$ [deg] & $v_{\max} - v_{\min}$ [deg] & RMSE \\
\hline
1st  & 4.041 & 4.776 & $-106.70$ & $-99.43$   & 7.27   & 0.0226 \\
5th  & 4.041 & 4.776 & 4.75      & 40.05      & 35.30  & 0.0037 \\
7th  & 4.041 & 4.776 & $-22.77$  & 27.51      & 50.28  & 0.0029 \\
11th & 4.041 & 4.776 & 79.69     & 164.01     & 84.32  & 0.0014 \\
13th & 4.041 & 4.776 & 49.38     & 147.67     & 98.29  & 0.0011 \\
\hline
...
The comparison plots between the empirical histograms and the denormalized shared Beta PDFs for each harmonic phase angle are shown in Fig.~\ref{fig:Shared_Beta_Fit}. Both the visual comparison and the low RMSE values indicate a good fit of the shared Beta distribution to the phase angle data across all harmonics.

\begin{figure*}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/Shared_Beta_Fit.png}
    \caption{Empirical histograms and denormalized shared Beta PDFs for harmonic phase angles.}
