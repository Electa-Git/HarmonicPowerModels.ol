...
\begin{IEEEeqnarray}{ 0 l }
    ...
    \quad \max \sum_{\Unit \in \FixedUnitSet}\sum_{\Harmonic \in \HarmonicSet \backslash \{1\}}  \CurrentMagnitude_{\Unit,\Harmonic} \\
    ...
    \text{lifted voltage variable --~}  \forall \Node \in \NodeSet, \Harmonic \in \HarmonicSet \nonumber \\
    \quad \LiftedVoltage_{\Node,\Harmonic} = (\RealVoltage_{\Node,\Harmonic})^{2} + (\ImaginaryVoltage_{\Node,\Harmonic})^{2}, \\
    \text{root mean square voltage limit --~} \forall \Node \in \NodeSet: \nonumber \\
    \quad (\MinimumVoltage_{\Node})^{2} \leq \mkern-7mu \sum_{\Harmonic \in \HarmonicSet} \LiftedVoltage_{\Node,\Harmonic} \leq (\MaximumVoltage_{\Node})^{2}, \label{eq:voltage_bounds_fund} \\
    \text{total harmonic voltage limit --~} \forall \Node \in \NodeSet: \nonumber \\
    \quad \sum_{\Harmonic \in \HarmonicSet \backslash \{1\}} \LiftedVoltage_{\Node,\Harmonic} \leq (\text{THD}_{\Node}^{\text{u}})^2\, \LiftedVoltage_{\Node,1}, \label{eq:thd_bounds} \\
    \text{individual harmonic voltage limit --~} \forall \Node \in \NodeSet, \Harmonic \in \HarmonicSet \backslash \{1\}: \nonumber \\
    \quad \LiftedVoltage_{\Node,\Harmonic} \leq (\text{IDH}^{\text{u}}_{\Node,\Harmonic})^{2}\, \LiftedVoltage_{\Node,1},
    ...
    \quad \sum_{\Harmonic \in \HarmonicSet} \left(   (\RealCurrent_{\LineTuple,\Harmonic})^{2} + (\ImaginaryCurrent_{\LineTuple,\Harmonic})^{2} \right) \leq (\RatedCurrent_{\LineTuple})^{2}, \\
    ...
    \textsc{Generator Constraints} \nonumber \\
    \text{generator power limits --~}    \forall \UnitTuple \in \DispatchableUnitTupleSet, \Harmonic \in \HarmonicSet: \nonumber\\
    \quad 
    \MinimumRealPower_{\Unit,\Harmonic} \leq 
    \RealPower_{\Unit,\Harmonic} \defeq \RealVoltage_{\Node,\Harmonic} \RealCurrent_{\Unit,\Harmonic} + 
    \ImaginaryVoltage_{\Node,\Harmonic} \ImaginaryCurrent_{\Unit,\Harmonic} \leq
    \MaximumRealPower_{\Unit,\Harmonic}, \\
    \quad 
    \MinimumImaginaryPower_{\Unit,\Harmonic} \leq \ImaginaryPower_{\Unit,\Harmonic} \defeq 
    \ImaginaryVoltage_{\Node,\Harmonic} \RealCurrent_{\Unit,\Harmonic} - 
    \RealVoltage_{\Node,\Harmonic} \ImaginaryCurrent_{\Unit,\Harmonic} \leq
    \MaximumImaginaryPower_{\Unit,\Harmonic}, \\
    \textsc{Harmonic Load Constraints} \nonumber \\
    \text{fundamental component --~} \forall \UnitTuple \in \FixedUnitTupleSet: \nonumber \\
    \quad 
    \RealPowerParameter_{\Unit,1} = 
    \RealVoltage_{\Node,1} \, \RealCurrent_{\Unit,1} + 
    \ImaginaryVoltage_{\Node,1} \, \ImaginaryCurrent_{\Unit,1}, \\
    \quad 
    \ImaginaryPowerParameter_{\Unit,1} = 
    \ImaginaryVoltage_{\Node,1} \, \RealCurrent_{\Unit,1} - 
    \RealVoltage_{\Node,1} \, \ImaginaryCurrent_{\Unit,1}, \\
    \text{harmonic component --~} \forall \UnitTuple \in \FixedUnitTupleSet, \Harmonic \in \HarmonicSet \backslash \{1\}: \nonumber \\
